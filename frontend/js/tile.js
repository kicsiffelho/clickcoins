...
export async function setBackgroundColor() {
  ...
  if (user) {
    const userId = user.id;
    const colors = await fetchBackgroundColor(userId);
    if (colors && Array.isArray(colors)) {
      colors.forEach((color) => updateGameAreaBackground(color));
    }
  } else {
...
document.addEventListener("DOMContentLoaded", function () {
  const bgButton = document.querySelectorAll(
    "#bgBlue, #bgBrown, #bgCrimson, #bgGreen, #bgGrey, #bgOrange, #bgPink, #bgRed"
  );
    ...
    document.querySelectorAll(".btn-secondary").forEach((button) => {
      ...
      if (ownedBackgrounds.includes(buttonId)) {
        ...
        button.addEventListener("click", () =>
          handlePurchase(buttonId, userId)
        );
      }
    });
  } catch (error) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  bgButton.forEach(button => {
      button.addEventListener('click', function(event) {
          event.preventDefault();
          const color = getColorById(button.id);
          if (!colors.includes(color)) {
              let price = getPriceById(button.id);
              changeBackgroundColor(color, price);
          } else {
              alert('You already own this color!');
          }
      });
  });
  setBackgroundColor();
